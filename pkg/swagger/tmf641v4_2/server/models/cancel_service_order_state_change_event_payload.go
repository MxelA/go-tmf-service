// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// CancelServiceOrderStateChangeEventPayload The event data structure
//
// swagger:model CancelServiceOrderStateChangeEventPayload
type CancelServiceOrderStateChangeEventPayload struct {

	// The involved resource data for the event
	CancelServiceOrder *CancelServiceOrder `json:"cancelServiceOrder,omitempty"`
}

// Validate validates this cancel service order state change event payload
func (m *CancelServiceOrderStateChangeEventPayload) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateCancelServiceOrder(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *CancelServiceOrderStateChangeEventPayload) validateCancelServiceOrder(formats strfmt.Registry) error {
	if swag.IsZero(m.CancelServiceOrder) { // not required
		return nil
	}

	if m.CancelServiceOrder != nil {
		if err := m.CancelServiceOrder.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("cancelServiceOrder")
			}
			return err
		}
	}

	return nil
}

// ContextValidate validate this cancel service order state change event payload based on the context it is used
func (m *CancelServiceOrderStateChangeEventPayload) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	if err := m.contextValidateCancelServiceOrder(ctx, formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *CancelServiceOrderStateChangeEventPayload) contextValidateCancelServiceOrder(ctx context.Context, formats strfmt.Registry) error {

	if m.CancelServiceOrder != nil {

		if swag.IsZero(m.CancelServiceOrder) { // not required
			return nil
		}

		if err := m.CancelServiceOrder.ContextValidate(ctx, formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("cancelServiceOrder")
			}
			return err
		}
	}

	return nil
}

// MarshalBinary interface implementation
func (m *CancelServiceOrderStateChangeEventPayload) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *CancelServiceOrderStateChangeEventPayload) UnmarshalBinary(b []byte) error {
	var res CancelServiceOrderStateChangeEventPayload
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
